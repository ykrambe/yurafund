...
func (r *repository) FindWithLimit(limit int) ([]Campaign, error) {
	var campaigns []Campaign
	err := r.db.Preload("CampaignImages", "campaign_images.is_primary = 1").Limit(limit).Find(&campaigns).Error
	if err != nil {
		return campaigns, err
	}
	return campaigns, nil
...
func (r *repository) MarkAllImagesAsNonPrimary(campaignID int) (bool, error) {
	err := r.db.Model(&CampaignImage{}).Where("campaign_id = ? AND is_primary = 1", campaignID).Update("is_primary", false).Error
	if err != nil {
		return false, err
	}
	return true, err
